...
class MainStackContainer extends Component {
  ...
  render() {
    ...
    const navigatorConfig = {
      ...
      tabBar: (props) => {
				const p = navigation.getState();
				const rr = getPathFromState(p);
				return rr.includes('LessonScreen') || rname === 'Invoice' ? null : (
          <View
            style={{
              position: "relative",
    ...
    const routeConfigs = {
        ...
        navigationOptions: ({ navigation }) => ({
          ...
          tabBarIcon: () => {
            ...
            return focused ? (
              <View key={2}>
                <SvgUri
                  uri={`${apiUrl}${theme.NOTIFICATIONS_ACTIVE_ICON.url}`}
                  stroke={"#3783F5"}
                />
      ...
      Invoice: {
        ...
        navigationOptions: ({ navigation }) => ({
          ...
          tabBarIcon: () => (
            <SvgUri
              uri={`${apiUrl}${theme.BIG_PLUS_ICON.url}`}
              fill="#FFFFFF"
              color="#FFFFFF"
              style={{ bottom: 20 }}
            />
      ...
      Events: {
        ...
        screen: EventsScreen,
        path: "/events",
        navigationOptions: ({ navigation, route: _r }) => {
          const srn = getFocusedRouteNameFromRoute(_r);
          return {
            title: "События",
            tabBarVisible: srn !== "LessonScreen",
            tabBarIcon: () => {
              const rn = getFocusedRouteNameFromRoute(route);
              ...
              return focused ? (
                <View key={2}>
                  <SvgUri
                    uri={`${apiUrl}${theme.EVENTS_ICON.url}`}
                    stroke={"#3783F5"}
                  />
